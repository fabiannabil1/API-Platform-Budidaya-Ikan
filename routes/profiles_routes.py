            ...
            if profile_exists:
                ...
                cur.execute("""
                    ...
                    SET address = COALESCE(%s, address),
                        ...
                        bio = COALESCE(%s, bio)
                    WHERE user_id = %s
                """, (data.get("address"), data.get("profile_picture"), data.get("bio"), current_user))
            else:
                # Create new profile
                cur.execute("""
                    INSERT INTO user_profiles (user_id, address, profile_picture, bio)
                    VALUES (%s, %s, %s, %s)
                """, (current_user, data.get("address"), data.get("profile_picture"), data.get("bio")))
